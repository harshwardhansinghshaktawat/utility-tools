class MemeGenerator extends HTMLElement {
  ...
  // ===== NEW: Observed attributes for CMS integration =====
  static get observedAttributes() {
    return ['cms-template-url', 'cms-template-name', 'cms-template-layout'];
  }

  // ===== NEW: Handle attribute changes from CMS =====
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    } else if (name === 'cms-template-name' && this.getAttribute('cms-template-url')) {
      this.setCMSTemplate(
        this.getAttribute('cms-template-url'),
        newValue,
        this.getAttribute('cms-template-layout') || '2-vertical'
      );
    } else if (name === 'cms-template-layout' && this.getAttribute('cms-template-url')) {
      this.setCMSTemplate(
        this.getAttribute('cms-template-url'),
        this.getAttribute('cms-template-name') || 'CMS Template',
        newValue
  ...
  loadTemplates() {
    ...
    this.state.templates.forEach(template => {
      ...
      // Add special styling for CMS template
