class MemeGenerator extends HTMLElement {
  constructor() {
    ...
    this.state = {
      ...
      textLayers: [],
      shapes: [],
      speechBubbles: [], // New array to store added speech bubbles
      uploadedImages: [],
      canvasWidth: 600,
      canvasHeight: 600,
      undoStack: [],
      ...
      // To add your own templates, add an entry to the templates array below
      // Format: 
      // { 
      //   id: 'unique-id', 
      //   name: 'Display Name', 
      ...
      // 
      // layout options: 
      // - '2-vertical': Two text boxes vertically aligned (top/bottom)
      // - '3-horizontal': Three text boxes horizontally aligned
      // - '4-vertical': Four text boxes vertically stacked
      // - '2-button': Two button-style text areas
      // - 'custom': No predefined text positions
      // 
      // Example of adding your custom template:
      // { 
      //   id: 'my-custom-meme', 
      //   name: 'My Funny Meme', 
  ...
  saveState() {
    this.state.undoStack.push(JSON.stringify({
      ...
      speechBubbles: this.state.speechBubbles // Add speech bubbles to the state saving
    }));
  ...
  undo() {
    if (this.state.undoStack.length > 0) {
      ...
      this.state.speechBubbles = prevState.speechBubbles || []; // Handle backward compatibility

      this.renderCanvas();
      this.renderTextLayersUI();
      this.renderImageLayersUI();
      this.renderShapeLayersUI();
      this.renderSpeechBubblesUI(); // Render speech bubbles UI
  ...
  redo() {
    if (this.state.redoStack.length > 0) {
      ...
      this.state.speechBubbles = redoneState.speechBubbles || []; // Handle backward compatibility
  ...
  renderCanvas() {
    ...
    // Draw background (either template or white)
  ...
  downloadMeme(format = 'png') {
    ...
    try {
      ...
      if (this.isCanvasTainted()) {
        alert('Cannot download due to cross-origin images. Try using a custom uploaded image or a different template.');
        this.isDownloading = false;
        return;
      }
